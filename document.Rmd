...
To access the app via web, you could click <https://wangzhili.shinyapps.io/shinyapp/>.
This document would introduce you to this app.

## 1 What can this app do?

As a big fan of Linkin Park, I want to analyze the lyrics of 20 of their songs so to find out what words they like for the most.
...
![Screenshot of the Word Cloud App](app.JPG)

### Instruction

Users could adjust the word cloud plot by sliding the two bars on the left: 

1. To change the minimum frequency of words to analyze, the user could slide the first bar so to adjust the choice of words, i.e. only words with frequency higher than the selected value could be shown in the word cloud. 
...
library(knitr)
data <- data.frame(No = 1:20, song_name = c("1 stp Klosr","Burn it down", "Crawling","Faint","Good Goodbye","In the End","Iridescent","Bleed it out","Burning in the skies","Breaking the habit","All for nothing","Forgotten","Hunger Strike","Invisible","Leave out all the rest","Numb","One Step Closer","Part of me","Rolling in the deep","Robot Boy"))
kable(data, caption = "List of Songs")
```
